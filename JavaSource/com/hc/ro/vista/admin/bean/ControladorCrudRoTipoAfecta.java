package com.hc.ro.vista.admin.bean;

import java.util.ArrayList;
import java.util.List;

import javax.annotation.PostConstruct;
import javax.ejb.EJB;
import javax.faces.bean.ManagedBean;
import javax.faces.bean.ManagedProperty;
import javax.faces.bean.ViewScoped;
import javax.faces.application.FacesMessage;
import javax.faces.context.FacesContext;

import org.primefaces.context.RequestContext;

import com.hc.ro.dataManager.DataManagerSesion;
import com.hc.ro.modelo.GenEstado;
import com.hc.ro.modelo.RoTipoAfecta;
import com.hc.ro.negocio.ServicioGenEstado;
import com.hc.ro.negocio.ServicioRoTipoAfecta;

@ManagedBean
@ViewScoped
public class ControladorCrudRoTipoAfecta {
	@ManagedProperty("#{controladorMenuPrincipal}")
	ControladorMenuPrincipal controladorMenuPrincipal;

	@ManagedProperty("#{dataManagerSesion}")
	DataManagerSesion dataManagerSesion;

	public final static String nombrePagina = "/paginas/CrudTipoAfecta.jsf";
	// TipoAfecta
	@EJB
	ServicioRoTipoAfecta servicioRoTipoAfecta;
	@EJB
	ServicioGenEstado servicioGenEstado;

	// VARIABLES
	private RoTipoAfecta roTipoAfectaVista;
	private RoTipoAfecta roTipoAfectaControlador;
	private List<RoTipoAfecta> tipoAfectasTodos;
	private List<GenEstado> estadosTodos;
	private int idTipoAfectaSeleccionado;
	private int idEstadoSeleccionado;
	private boolean tipoGuardar;
	private String nombreSeleccionado;
	// botones,cajas,dialogos
	private boolean btnAnadir;
	private boolean btnCancelar;
	private boolean btnGuardar;
	private boolean pnlTipoAfecta;

	//
	public ControladorCrudRoTipoAfecta() {
		super();
		roTipoAfectaControlador = new RoTipoAfecta();
		roTipoAfectaVista = new RoTipoAfecta();
		tipoAfectasTodos = new ArrayList<RoTipoAfecta>();
		estadosTodos = new ArrayList<GenEstado>();
		nombreSeleccionado = new String();
	}

	@PostConstruct
	public void PostControladorCrudRoTipoAfecta() {
		try {
			controladorMenuPrincipal.controlarAcceso();

		} catch (Exception e) {
			// TODO: handle exception
		}
		try {
			dataManagerSesion.controlarAcceso(nombrePagina);

		} catch (Exception e) {
			// TODO: handle exception
		}
		tipoAfectasTodos = servicioRoTipoAfecta.buscarTodos();
		estadosTodos = servicioGenEstado.buscarTodos();
		btnAnadir = false;
		btnCancelar = true;
		btnGuardar = true;
		pnlTipoAfecta = false;
	}

	// METODOS

	public void guardarTipoAfecta() {
		roTipoAfectaVista.setGenEstado(servicioGenEstado
				.buscarPorId(idEstadoSeleccionado));
		if (tipoGuardar) {
			if (servicioRoTipoAfecta
					.existeTipoAfectaPorNombre(roTipoAfectaVista
							.getNombreTafc())) {
				servicioRoTipoAfecta.insertar(roTipoAfectaVista);
				FacesContext context = FacesContext.getCurrentInstance();

				context.addMessage(null, new FacesMessage(
						"Tipo de Afectación Añadido",
						"El Tipo de Afectación ha sido Añadido con éxito"));
				exitoGuardar();
			} else {
				FacesContext context = FacesContext.getCurrentInstance();

				context.addMessage(null, new FacesMessage(
						FacesMessage.SEVERITY_ERROR, "Error al guardar:",
						"El nombre de Tipo de Afectación no se deben repetir"));
			}
		} else {

			if (servicioRoTipoAfecta.existeTipoAfectaPorNombreEx(
					roTipoAfectaVista.getNombreTafc(),
					roTipoAfectaVista.getCodigoTafc())) {

				servicioRoTipoAfecta.actualizar(roTipoAfectaVista);

				FacesContext context = FacesContext.getCurrentInstance();

				context.addMessage(null, new FacesMessage(
						"Tipo de Afectación Actualizado",
						"El Tipo de Afectación ha sido Actualizado con éxito"));
				exitoGuardar();
			} else {
				FacesContext context = FacesContext.getCurrentInstance();

				context.addMessage(null, new FacesMessage(
						FacesMessage.SEVERITY_ERROR, "Error al guardar:",
						"El nombre de Tipo de Afectación no se deben repetir"));
				roTipoAfectaVista.setNombreTafc(roTipoAfectaControlador
						.getNombreTafc());
			}
		}
	}

	public void exitoGuardar() {

		roTipoAfectaVista = new RoTipoAfecta();
		idEstadoSeleccionado = 1;
		tipoAfectasTodos = servicioRoTipoAfecta.buscarTodos();
		btnAnadir = false;
		btnGuardar = true;
		btnCancelar = true;
		pnlTipoAfecta = false;
	}

	public void eliminarTipoAfecta() {

		try {
			servicioRoTipoAfecta.eliminar(roTipoAfectaVista);
			roTipoAfectaVista = new RoTipoAfecta();
			tipoAfectasTodos = servicioRoTipoAfecta.buscarTodos();
			idEstadoSeleccionado = 1;
			btnGuardar = true;
			btnCancelar = true;
			pnlTipoAfecta = false;
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(
					"Tipo de Afectación Eliminado",
					"El Tipo de Afectación ha sido Eliminado con éxito"));
		} catch (Exception e) {
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(null, new FacesMessage(
					FacesMessage.SEVERITY_ERROR, "Error al eliminar:",
					"Tipo de Afectación en uso"));
		}
	}

	public void nuevoTipoAfectaVista() {
		tipoGuardar = true;
		roTipoAfectaVista = new RoTipoAfecta();
		idEstadoSeleccionado = 1;
		btnGuardar = false;
		btnCancelar = false;
		pnlTipoAfecta = true;
	}

	public void cancelar() {
		nuevoTipoAfectaVista();
		btnGuardar = true;
		btnAnadir = false;
		btnCancelar = true;
		pnlTipoAfecta = false;
		RequestContext.getCurrentInstance().update("formTipoAfecta");
	}

	public void editarTipoAfectaVista() {
		roTipoAfectaControlador
				.setNombreTafc(roTipoAfectaVista.getNombreTafc());
		tipoGuardar = false;
		idEstadoSeleccionado = (int) roTipoAfectaVista.getGenEstado().getCodigoEsta();

		btnGuardar = false;
		btnCancelar = false;
		pnlTipoAfecta = true;
		btnAnadir = true;

	}

	// GETTERS Y SETTERS

	public boolean isBtnAnadir() {
		return btnAnadir;
	}

	public void setBtnAnadir(boolean btnAnadir) {
		this.btnAnadir = btnAnadir;
	}

	public boolean isBtnCancelar() {
		return btnCancelar;
	}

	public void setBtnCancelar(boolean btnCancelar) {
		this.btnCancelar = btnCancelar;
	}

	public boolean isBtnGuardar() {
		return btnGuardar;
	}

	public void setBtnGuardar(boolean btnGuardar) {
		this.btnGuardar = btnGuardar;
	}

	public RoTipoAfecta getRoTipoAfectaVista() {
		return roTipoAfectaVista;
	}

	public void setRoTipoAfectaVista(RoTipoAfecta roTipoAfectaVista) {
		this.roTipoAfectaVista = roTipoAfectaVista;
	}

	public RoTipoAfecta getRoTipoAfectaControlador() {
		return roTipoAfectaControlador;
	}

	public void setRoTipoAfectaControlador(RoTipoAfecta roTipoAfectaControlador) {
		this.roTipoAfectaControlador = roTipoAfectaControlador;
	}

	public List<RoTipoAfecta> getTipoAfectasTodos() {
		return tipoAfectasTodos;
	}

	public void setTipoAfectasTodos(List<RoTipoAfecta> TipoAfectasTodos) {
		this.tipoAfectasTodos = TipoAfectasTodos;
	}

	public int getIdTipoAfectaSeleccionado() {
		return idTipoAfectaSeleccionado;
	}

	public void setIdTipoAfectaSeleccionado(int idTipoAfectaSeleccionado) {
		this.idTipoAfectaSeleccionado = idTipoAfectaSeleccionado;
	}

	public boolean isTipoGuardar() {
		return tipoGuardar;
	}

	public void setTipoGuardar(boolean tipoGuardar) {
		this.tipoGuardar = tipoGuardar;
	}

	public List<GenEstado> getEstadosTodos() {
		return estadosTodos;
	}

	public void setEstadosTodos(List<GenEstado> estadosTodos) {
		this.estadosTodos = estadosTodos;
	}

	public int getIdEstadoSeleccionado() {
		return idEstadoSeleccionado;
	}

	public void setIdEstadoSeleccionado(int idEstadoSeleccionado) {
		this.idEstadoSeleccionado = idEstadoSeleccionado;
	}

	public String getNombreSeleccionado() {
		return nombreSeleccionado;
	}

	public void setNombreSeleccionado(String nombreSeleccionado) {
		this.nombreSeleccionado = nombreSeleccionado;
	}

	public boolean isPnlTipoAfecta() {
		return pnlTipoAfecta;
	}

	public void setPnlTipoAfecta(boolean pnlTipoAfecta) {
		this.pnlTipoAfecta = pnlTipoAfecta;
	}

	public ControladorMenuPrincipal getControladorMenuPrincipal() {
		return controladorMenuPrincipal;
	}

	public void setControladorMenuPrincipal(
			ControladorMenuPrincipal controladorMenuPrincipal) {
		this.controladorMenuPrincipal = controladorMenuPrincipal;
	}

	public DataManagerSesion getDataManagerSesion() {
		return dataManagerSesion;
	}

	public void setDataManagerSesion(DataManagerSesion dataManagerSesion) {
		this.dataManagerSesion = dataManagerSesion;
	}

}
