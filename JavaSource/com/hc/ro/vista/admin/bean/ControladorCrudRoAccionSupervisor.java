...
@ManagedBean
@ViewScoped
public class ControladorCrudRoAccionSupervisor {
	@EJB
	...
	public static final String nombrePagina = "/paginas/CrudAccionSupervisor.jsf";
	...
	private List<TimelineEvent> overlappedOrders; // all overlapped orders
													// (events) to the changed
													// order (event)
	private List<TimelineEvent> ordersToMerge; // selected orders (events) in
												// the dialog which should be
												// merged

	public ControladorCrudRoAccionSupervisor() {
		...
		overlappedOrders = new ArrayList<TimelineEvent>();
		ordersToMerge = new ArrayList<TimelineEvent>();
	}
	...
	public void PostControladorCrudRoAccion() {
		...
		try {
			...
			this.accionesTodos = this.servicioRoAccion.buscarTodos();
	...
	public void agruparPlanesPorResponsable() {
		...
		for (RoDetalleAccion roDeac : roDetalleAccionTodos) {
			...
			model.add(new TimelineEvent(roDeac.getNombreDeac(), startReport,
	...
	public void onSelect(TimelineModificationEvent e) {
		TimelineEvent timelineEvent = e.getTimelineEvent();
		DateFormat df4 = DateFormat.getDateInstance(DateFormat.FULL);
		FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,
				timelineEvent.getData().toString(), df4.format(timelineEvent
						.getStartDate())
						+ " - "
						+ df4.format(timelineEvent.getEndDate()));
	...
	public void nuevoAccion() {
		...
		this.roAccionVista.setRiesgoInherente(new BigDecimal(Float
				.toString(this.roDetalleEventoVista.getValorDeve())).setScale(
				2, 4));
		if (this.roDetalleEventoVista.getTipoCalifDeve().equals("Cuantitativo")) {
			this.roAccionVista.setNumConsecResidual(Integer
					.toString(this.roDetalleEventoVista
							.getParamConsecuenciaImpactoDespues()
							.getNumeroPconi()));
			this.roAccionVista.setNumProbaResidual(this.roDetalleEventoVista
			...
			this.roAccionVista.setNumConsecInherente(Integer
					.toString(this.roDetalleEventoVista
							...
							.getNumeroPconi()));
			this.roAccionVista.setNumProbaInherente(this.roDetalleEventoVista
	...
	public void seleccionar() {
		...
		this.roDetalleAccionTodos = this.servicioRoDetalleAccion
				.buscarRoDetalleAccionPorSupervisorFecha(this.dataManagerSesion
						.getUsuarioSesion().getCodigoUsua(), fechaInicio,
						fechaFin);
	...
	public void guardarDetalleAccion() {
		...
		if (this.tipoGuardarDetalle) {
			...
			context.addMessage(null, new FacesMessage(
					"Detalle de Plan de Acción Modificado",
					"El Detalle de Plan de Acción ha sido Modificado con Exito"));
			finalizarGuardadoDetalle();
		}
	}
...
//		for (int i=0; i< this.accionesTodos.size(); i++){
//			System.out.println(this.accionesTodos.get(i).getCodigoAcci());
//		}
...
//		roAccionVista = new RoAccion();
		...
		System.out.println(this.roDetalleAccionTodos.get(i));
		}
...
//		}
	...
	public void setOverlappedOrders(List<TimelineEvent> overlappedOrders) {
		this.overlappedOrders = overlappedOrders;
	}
