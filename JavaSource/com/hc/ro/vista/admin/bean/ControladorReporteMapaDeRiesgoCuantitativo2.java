...
@ManagedBean
@ViewScoped
public class ControladorReporteMapaDeRiesgoCuantitativo2 {

	public final static String nombrePagina = "/paginas/ReporteMapaDeRiesgoCuantitativo2.jsf";
	@EJB
	...
	// Listas para exportar cabecera pdf
	...
	public ControladorReporteMapaDeRiesgoCuantitativo2() {
		super();
...
//		usuariosTodos = servicioSisUsuario.buscarTodos_codigoUsua_nombreUsua();
		...
		System.out.println("empezamos pc 10");
		llenarArbolAgencia();
		System.out.println("empezamos pc 11");
		llenarArbolDepartamento();
		System.out.println("empezamos pc 12");
		llenarArbolEvento();
		System.out.println("empezamos pc 13");
		llenarArbolNegocio();
		System.out.println("empezamos pc 14");
		llenarArbolProceso();
		System.out.println("empezamos pc 15");
		pintarMapa();
	...
	public void crearReporteMapa(ActionEvent actionEvent) {
		...
		try {
			...
			// seteamos el titulo del reporte
			Paragraph parrafoTitulo = new Paragraph(
					"Reporte Mapa de Riesgo Cuantitativo 2",
			...
			// parrafo.add(frase);
			//
			// frase = new Phrase(tipoMapa + ", ", FontFactory.getFont("arial",
			// 8));
			// parrafo.add(frase);
			...
			//
			// parrafo = new Paragraph();
			// frase = new Phrase("Departamentos: ",
			// FontFactory.getFont("arial",
			// 8, Font.BOLD));
			// // parrafo.add(frase);
			...
			tabla.setWidths(new int[] { 3, 1, 1, 1, 1, 1 });
			tabla.addCell(new Phrase("Evento", FontFactory.getFont("arial", 12,
					Font.BOLD)));
			tabla.addCell(new Phrase("Ocurrencias", FontFactory.getFont(
					"arial", 12, Font.BOLD)));
			tabla.addCell(new Phrase("Probabilidad", FontFactory.getFont(
					"arial", 12, Font.BOLD)));
			tabla.addCell(new Phrase("Consecuencia Impacto", FontFactory
					.getFont("arial", 12, Font.BOLD)));
			tabla.addCell(new Phrase("Calificación", FontFactory.getFont(
					"arial", 12, Font.BOLD)));
			tabla.addCell(new Phrase("Num. Detalles", FontFactory.getFont(
					"arial", 12, Font.BOLD)));
			...
			for (CruceMapaDeRiesgo cruce : cruceMapaTodos) {
				...
				for (RoEvento evento : cruce.getRoEventos()) {
					...
					tabla2.addCell(cell);
					// tabla2.addCell(evento.getNombreEven());
					tabla2.addCell(""
							+ Math.round(evento.getValorPerdidaEvento() * 100.0)
							/ 100.0);
				...
				tabla.addCell("" + cruce.getRoEventos().size());
				tabla.addCell(""
						+ cruce.getParamProbabilidadRiesgo().getNombrePprr()
						+ " ("
						+ cruce.getParamProbabilidadRiesgo().getNumeroPprr()
						+ ")");
				tabla.addCell(""
						+ cruce.getParamConsecuenciaImpacto().getNombrePconi()
						+ " ("
						+ cruce.getParamConsecuenciaImpacto().getNumeroPconi()
						+ ")");
				// tabla.addCell(""
				...
				// < fila 5
				cell = new PdfPCell(new Phrase(cruce.getRoAriesgo()
						.getRoCalifRiesgo().getNombreClrs()));
				cell.setBackgroundColor(new Color(Integer.parseInt(cruce
						.getRoAriesgo().getRoCalifRiesgo().getColorClrs(), 16)));
				tabla.addCell(cell);
				// < fila 5

				tabla.addCell("" + cruce.getDetalleEventos().size());
			...
			//
			// // probabilidades
			// for (ParamProbabilidadRiesgo proba :
			// paramProbabilidadRiesgoTodos) {
			...
			// tablaMapa.addCell("");
			// tablaProba = new PdfPTable(columnas);
			// for (ParamConsecuenciaImpacto consec :
			// paramConsecuenciaImpactoTodos) {
			//
	...
	public void generarMapa() {
		...
		for (ParamProbabilidadRiesgo proba : paramProbabilidadRiesgoTodos) {
			for (ParamConsecuenciaImpacto consec : paramConsecuenciaImpactoTodos) {
				...
				for (RoEvento detalle : eventosMapa) {
					if (detalle.getNumConsecuencia() == consec.getNumeroPconi()
							&& detalle.getNumProbabilidad() == proba
									.getNumeroPprr()) {
						cruceMapaAux.getRoEventos().add(detalle);
						for (RoDetalleEvento deve : detalle.getDetallesEvento()) {
				...
				if (cruceMapaAux.getRoEventos().size() > 0) {
	...
	public void seleccionarEvento() {
		System.out.println("-ro evento controlador: "
				+ roEventoControlador.getNombreEven());
		System.out.println("-ro evento controlador deta size: "
				+ roEventoControlador.getDetallesEvento().size());
		roEventoVista = new RoEvento();
		detallesEventoSeleccionados = new ArrayList<RoDetalleEvento>();
		roEventoVista = roEventoControlador;
		dataManagerSesion.setNumeroDeOcurrencias(roEventoVista
				.getDetallesEvento().size());
		// for (RoDetalleEvento detalleEvento :
		// cruceMapaVista.getDetalleEventos()) {
		for (RoDetalleEvento detalleEvento : roEventoVista.getDetallesEvento()) {
